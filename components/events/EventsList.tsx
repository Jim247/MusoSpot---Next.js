...
import { getEventDate } from '../../lib/utils/utils'
    ...
    <div className="mt-8 bg-white rounded-lg p-6 col-2 shadow-md">
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          <a href="/addevent" className="text-highlight hover:text-secondary text-sm">
            + Post New Event
          </a>
        </div>
        {events.length > 0 ? (
              ...
              <div key={event.id} className="border rounded-lg p-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p>Date: {getEventDate(event.date)}</p>
                    <p>Location: {event.postcode}</p>
                    <p>Budget: £{event.budget}</p>
                    <p>Looking for:</p>
                    <div className="flex flex-wrap gap-2 mt-1">
                      {event.instrumentsNeeded.map((instrument: string, idx: number) => (
